...
@Autonomous(name = "PedroPath_Auton_Ver2", group = "Examples")
public class Pedropath_Auton_Ver2 extends OpMode {
    ...
    private final Pose scorePose = new Pose(8, 132, Math.toRadians(-45));
    //private final Pose scorePose = new Pose(19, 111);

    /** Lowest (First) Sample from the Spike Mark */
    private final Pose pickup1Pose = new Pose(26, 131);
    /** Lowest (First) Sample from the Spike Mark */
    private final Pose intake1Pose = new Pose(10, 131);

    private final Pose score1Pose = new Pose(9, 138, Math.toRadians(-45));

    private final Pose intake2Pose = new Pose(10, 140);

    /** Middle (Second) Sample from the Spike Mark */
    private final Pose pickup2Pose = new Pose(26, 140);

    private final Pose score2Pose = new Pose(10, 140, Math.toRadians(-60));

    /** Highest (Third) Sample from the Spike Mark */
    private final Pose pickup3Pose = new Pose(22  , 140, Math.toRadians(100));
    private final Pose intake3Pose = new Pose(27  , 144, Math.toRadians(100));

    /** Park Pose for our robot, after we do all of the scoring. */
    private final Pose parkPose = new Pose(55, 70, Math.toRadians(-55));

    /** Park Control Pose for our robot, this is used to manipulate the bezier curve that we will create for the parking.
     * The Robot will not go to this pose, it is used a control point for our bezier curve. */
    private final Pose parkControlPose = new Pose(60, 98);

    /* These are our Paths and PathChains that we will define in buildPaths() */
    private Path scorePreload, park;
    private PathChain grabPickup1, grabPickup2, grabPickup3, scorePickup1, scorePickup2, scorePickup3, intakePickup1,intakePickup2,intakePickup3;
    ...
    public void buildPaths() {
        ...
        scorePickup3 = follower.pathBuilder()
                .addPath(new BezierLine(new Point(intake3Pose), new Point(score2Pose)))
                .setLinearHeadingInterpolation(intake3Pose.getHeading(), score2Pose.getHeading())
                .build();

        /* This is our park path. We are using a BezierCurve with 3 points, which is a curved line that is curved based off of the control point */
        park = new Path(new BezierCurve(new Point(score2Pose), /* Control Point */ new Point(parkControlPose), new Point(parkPose)));
    ...
    public void autonomousPathUpdate() {
        switch (pathState) {
            ...
            case 1:
                ...
                */
                ...
                if(!follower.isBusy()) {
                    ...
                    lift.elevate(0, outtake,opmodeTimer.getElapsedTime());
            ...
            case 4:
                ...
                // if(follower.getPose().getX() > (scorePose.getX() - 1) && follower.getPose().getY() > (scorePose.getY() - 1)) {
                if(!follower.isBusy()) {
                    boolean beamBroken =  outtake.grab(1);outtake.grab(1);
                    /* Score Sample */
                    // follower.breakFollowing();
                    lift.elevate(1, outtake,opmodeTimer.getElapsedTime());
                    //outtake.grab(2);
            ...
            case 11:
                ...
                if(follower.getPose().getX() > (parkPose.getX() - 1) && follower.getPose().getY() > (parkPose.getY() - 1)) {
                    ...
                    boolean beamBroken =  outtake.grab(3);
